#!/usr/bin/env bash
# ============================================================================
#  Al Islam wal Istibdad Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/11-al-islam-wal-istibdad}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.alislamwalistibdad}"
AVD_NAME="${AVD_NAME:-al_islam_wal_istibdad_test}"
...
cmd_test() {
  ...
  local out="/tmp/al-islam-wal-istibdad-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Al Islam wal Istibdad Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
